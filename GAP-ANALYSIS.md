...
| **mini-lux 现状** | 纯 slice 截断：`otherMsgs.slice(-keep)` + 工具结果 `slice(0, 1500)`。直接砍掉后半段，丢失语义 |
...
| **mini-lux 现状** | 完全没有注入机制。agent 只有主动调用 `recall` 才能搜索记忆。如果 LLM 没想到要 recall，就完全失忆 |
...
| **mini-lux 现状** | 任务只是"创建 + 标记状态"。LLM 需要自己记住要执行哪些任务，手动调用 update_task。没有任务驱动的执行流，没有依赖关系 |
...
| **mini-lux 现状** | 直接 `await this.client.chat.completions.create()`，无重试、无超时、无错误恢复。API 网络波动 → 直接崩溃 |
...
| **mini-lux 现状** | 纯截断：`content.slice(0, 1500)`。砍掉后半段，如果关键信息在后半段就丢失了 |
...
| **mini-lux 现状** | 只有 `write_file`（全量覆盖）。修改一个文件的某一行需要重写整个文件 |
...
| **mini-lux 现状** | `search_files` 只搜文件名，不搜文件内容 |
...
| **mini-lux 现状** | 只有 `execute_command`（Shell 命令）。可以跑 `node -e` 但没有沙箱，且违反 Lux 设计原则 |
...
| **mini-lux 现状** | `read_file`：读全文截断到 4000 字符。无行号、无 offset/limit |
...
| **mini-lux 现状** | 只有扁平的 memories 表（content + tags + embedding）。没有实体/关系/巩固 |
...
| **mini-lux 现状** | Persona 只有 system prompt + 工具列表，没有 skills 层 |
...
| **mini-lux 现状** | Web 页面（浏览器访问 localhost:3111） |
