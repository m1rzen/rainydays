...
test("GOV-02 version, persistence, Session and Link integration matrix is side-effect free", async () => {
  ...
  try {
    ...
    assert(payload.scenarios.some((entry) => entry.name === "fresh database migrates 0 to 5"));
    assert(payload.scenarios.some((entry) => entry.name === "Schema 1 migrates transactionally to 5"));
    ...
    assert(payload.scenarios.some((entry) => entry.name === "Schema 5 repeated startup is a migration no-op"));
