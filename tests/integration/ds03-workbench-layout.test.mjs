...
test("DS-03 schema 10 persists one canonical WorkbenchLayout with optimistic revision", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(payload(write.stdout), { mode: "write", schemaVersion: 10, revision: 2, tabs: 2 });
    const read = await runProcess(process.execPath, [child, "read", fixture], { cwd: projectRoot, timeoutMs: 15_000 });
    assert.equal(read.code, 0, read.stderr);
    assert.deepEqual(payload(read.stdout), { mode: "read", schemaVersion: 10, revision: 2, tabs: 2 });
  } finally {
