...
test("RT-07 Schema 2 migration preserves legacy tasks in the current Schema 11 DAG", async () => {
  ...
  try {
    ...
    try {
      ...
      database.exec(`
        DROP TABLE session_persona_bindings;
    ...
    assert.equal(parseLastJson(migrated.stdout).userVersion, 11);
...
test("RT-07 Task DAG is atomic, Session-scoped, fork-safe, and restart-persistent", async () => {
  ...
  try {
    ...
    assert.equal(persisted.schemaVersion, 11);
