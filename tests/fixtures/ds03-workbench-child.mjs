...
try {
  ...
  if (mode === "write") {
    ...
    console.log(JSON.stringify({ mode, schemaVersion: 10, revision: 2, tabs: secondLayout.root.tabs.length }));
  } else {
    ...
    console.log(JSON.stringify({ mode, schemaVersion: 10, revision: snapshot.revision, tabs: layout.root.tabs.length }));
  }
} finally {
  await db.closeDb();
}
