...
if (scenario === "normal") {
  ...
  assert.throws(() => db.validateDatabaseRestoreCandidate(mainPath, 4), /schema version is incompatible/iu);
  ...
  const lease = await store.stageValidatedDatabaseRestore(
    snapshot.bytes,
    candidate => db.validateDatabaseRestoreCandidate(candidate, 3)
  );
  ...
  const activeConnection = await openBootstrapDatabase(3);
  ...
  try {
    ...
    for (const variant of variants) {
      ...
      await assert.rejects(() => openBootstrapDatabase(3), /SQLite WAL/u, variant.name);
    ...
    await assert.rejects(() => openBootstrapDatabase(3), /SQLite WAL|PATH_IDENTITY_CHANGED/u, "hardlinked WAL");
    ...
    await assert.rejects(() => openBootstrapDatabase(3), /SQLite WAL|PATH_/u, "directory WAL");
    ...
    await restoreBaseline(baseline.wal);
    const recovered = await openBootstrapDatabase(3);
  ...
  const snapshot = await db.createConsistentDatabaseSnapshot();
  assert.equal(snapshot.validation.schemaVersion, 3);
