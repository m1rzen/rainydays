...
if (scenario === "normal") {
  ...
  const lease = await store.stageValidatedDatabaseRestore(
    snapshot.bytes,
    candidate => db.validateDatabaseRestoreCandidate(candidate, 6)
  );
  ...
  const activeConnection = await openBootstrapDatabase(6);
  ...
  try {
    ...
    for (const variant of variants) {
      ...
      await assert.rejects(() => openBootstrapDatabase(6), /SQLite WAL/u, variant.name);
    ...
    await assert.rejects(() => openBootstrapDatabase(6), /SQLite WAL|PATH_IDENTITY_CHANGED/u, "hardlinked WAL");
    ...
    await assert.rejects(() => openBootstrapDatabase(6), /SQLite WAL|PATH_/u, "directory WAL");
    ...
    await restoreBaseline(baseline.wal);
    const recovered = await openBootstrapDatabase(6);
  ...
  const snapshot = await db.createConsistentDatabaseSnapshot();
  assert.equal(snapshot.validation.schemaVersion, 6);
