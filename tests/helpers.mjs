...
import { loadRt11ResolvedManifest } from "../scripts/rt11-governance.mjs";
import { loadEvt01ResolvedManifest } from "../scripts/evt01-governance.mjs";
import { loadEvt02ResolvedManifest } from "../scripts/evt02-governance.mjs";
...
import { loadDs06ResolvedManifest } from "../scripts/ds06-governance.mjs";
import { loadDs07ResolvedManifest } from "../scripts/ds07-governance.mjs";
...
import { loadTool02ResolvedManifest } from "../scripts/tool02-governance.mjs";
...
import { loadRel04ResolvedManifest } from "../scripts/rel04-governance.mjs";
...
const expectedPersonaChains = Object.freeze({
  ...
  "TOOL-04": Object.freeze(["architect", "developer", "sentinel", "debugger", "reviewer"]),
  "TOOL-05": Object.freeze(["developer", "debugger", "reviewer"]),
  "TOOL-07": Object.freeze(["architect", "developer", "debugger", "reviewer"]),
...
async function loadRt11TaskView(root = projectRoot) {
  const resolved = await loadRt11ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "rt-11.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadEvt01TaskView(root = projectRoot) {
  const resolved = await loadEvt01ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "evt-01.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadEvt02TaskView(root = projectRoot) {
  const resolved = await loadEvt02ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "evt-02.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadEvt03TaskView(root = projectRoot) {
  const resolved = await loadEvt03ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "evt-03.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadDs08TaskView(root = projectRoot) {
  const resolved = await loadDs08ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "ds-08.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadDs09TaskView(root = projectRoot) {
  const resolved = await loadDs09ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "ds-09.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadDs10TaskView(root = projectRoot) {
  const resolved = await loadDs10ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "ds-10.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadTool02TaskView(root = projectRoot) {
  const resolved = await loadTool02ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "tool-02.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadTool04TaskView(root = projectRoot) {
  const resolved = await loadTool04ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "tool-04.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadTool05TaskView(root = projectRoot) {
  const resolved = await loadTool05ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "tool-05.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadTool07TaskView(root = projectRoot) {
  const resolved = await loadTool07ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "tool-07.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadRel04TaskView(root = projectRoot) {
  const resolved = await loadRel04ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "rel-04.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadDs06TaskView(root = projectRoot) {
  const resolved = await loadDs06ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "ds-06.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
async function loadDs03TaskView(root = projectRoot) {
  const resolved = await loadDs03ResolvedManifest({ root });
  const sourcePath = path.join(root, "tests", "manifests", "ds-03.json");
  const source = resolved.source;
  const layers = Object.fromEntries(layerNames.map(layer => [layer, []]));
  for (const record of resolved.manifest.testEntries) layers[record.layer].push(record.exactCasePath);
  return {
...
export async function loadTaskManifest(taskId = "GOV-03", root = projectRoot) {
  ...
  if (taskId === "RT-11") return loadRt11TaskView(root);
  if (taskId === "EVT-01") return loadEvt01TaskView(root);
  if (taskId === "EVT-02") return loadEvt02TaskView(root);
  ...
  if (taskId === "DS-06") return loadDs06TaskView(root);
  if (taskId === "DS-07") return loadDs07TaskView(root);
  ...
  if (taskId === "TOOL-02") return loadTool02TaskView(root);
  ...
  if (taskId === "REL-04") return loadRel04TaskView(root);
