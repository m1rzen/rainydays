...
import {
  ...
  loadDs06ResolvedManifest,
  readDs06BoundFile,
  validateDs06ResolvedManifest,
} from "../../scripts/ds06-governance.mjs";
import { publishDs06ResolvedBytes } from "../../scripts/generate-ds06-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-06 resolved governance binds exact File Tab runtime and evidence identities", async () => {
  const built = await buildDs06ResolvedManifest();
  const loaded = await loadDs06ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs06ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-06"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-06"));
});

test("DS-06 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs06ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs06ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs06ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs06ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-06 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds06-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs06BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs06BoundFile("live/manifest.json", {
        root,
...
test("DS-06 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds06-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs06ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-06 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds06-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs06ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
