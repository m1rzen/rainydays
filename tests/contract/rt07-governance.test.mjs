...
import {
  ...
  loadRt07ResolvedManifest,
  readRt07BoundFile,
  validateRt07ResolvedManifest,
} from "../../scripts/rt07-governance.mjs";
import { publishRt07ResolvedBytes } from "../../scripts/generate-rt07-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-07 resolved governance binds exact Task DAG runtime and evidence identities", async () => {
  const built = await buildRt07ResolvedManifest();
  const loaded = await loadRt07ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt07ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-07"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-07"));
});

test("RT-07 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt07ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt07ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt07ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt07ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-07 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt07-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt07BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt07BoundFile("live/manifest.json", {
        root,
...
test("RT-07 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt07-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt07ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-07 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt07-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt07ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
