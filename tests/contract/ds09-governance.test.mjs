...
import {
  ...
  loadDs09ResolvedManifest,
  readDs09BoundFile,
  validateDs09ResolvedManifest,
} from "../../scripts/ds09-governance.mjs";
import { publishDs09ResolvedBytes } from "../../scripts/generate-ds09-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-09 resolved governance binds exact Settings runtime and evidence identities", async () => {
  const built = await buildDs09ResolvedManifest();
  const loaded = await loadDs09ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs09ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-09"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-09"));
});

test("DS-09 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs09ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs09ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs09ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs09ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-09 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds09-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs09BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs09BoundFile("live/manifest.json", {
        root,
...
test("DS-09 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds09-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs09ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-09 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds09-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs09ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
