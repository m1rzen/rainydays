...
import {
  ...
  loadDs03ResolvedManifest,
  readDs03BoundFile,
  validateDs03ResolvedManifest,
} from "../../scripts/ds03-governance.mjs";
import { publishDs03ResolvedBytes } from "../../scripts/generate-ds03-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-03 resolved governance binds exact Workbench layout runtime and evidence identities", async () => {
  const built = await buildDs03ResolvedManifest();
  const loaded = await loadDs03ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs03ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-03"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-03"));
});

test("DS-03 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs03ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs03ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs03ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs03ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-03 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds03-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs03BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs03BoundFile("live/manifest.json", {
        root,
...
test("DS-03 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds03-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs03ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-03 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds03-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs03ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
