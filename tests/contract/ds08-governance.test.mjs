...
import {
  ...
  loadDs08ResolvedManifest,
  readDs08BoundFile,
  validateDs08ResolvedManifest,
} from "../../scripts/ds08-governance.mjs";
import { publishDs08ResolvedBytes } from "../../scripts/generate-ds08-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-08 resolved governance binds exact keyboard and input runtime and evidence identities", async () => {
  const built = await buildDs08ResolvedManifest();
  const loaded = await loadDs08ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs08ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-08"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-08"));
});

test("DS-08 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs08ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs08ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs08ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs08ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-08 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds08-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs08BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs08BoundFile("live/manifest.json", {
        root,
...
test("DS-08 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds08-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs08ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-08 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds08-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs08ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
