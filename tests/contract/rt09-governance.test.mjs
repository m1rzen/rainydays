...
import {
  ...
  loadRt09ResolvedManifest,
  readRt09BoundFile,
  validateRt09ResolvedManifest,
} from "../../scripts/rt09-governance.mjs";
import { publishRt09ResolvedBytes } from "../../scripts/generate-rt09-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-09 resolved governance binds exact Session semantics runtime and evidence identities", async () => {
  const built = await buildRt09ResolvedManifest();
  const loaded = await loadRt09ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt09ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-09"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-09"));
});

test("RT-09 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt09ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt09ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt09ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt09ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-09 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt09-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt09BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt09BoundFile("live/manifest.json", {
        root,
...
test("RT-09 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt09-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt09ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-09 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt09-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt09ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
