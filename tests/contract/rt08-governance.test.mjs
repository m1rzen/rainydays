...
import {
  ...
  loadRt08ResolvedManifest,
  readRt08BoundFile,
  validateRt08ResolvedManifest,
} from "../../scripts/rt08-governance.mjs";
import { publishRt08ResolvedBytes } from "../../scripts/generate-rt08-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-08 resolved governance binds exact Subagent lifecycle runtime and evidence identities", async () => {
  const built = await buildRt08ResolvedManifest();
  const loaded = await loadRt08ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt08ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-08"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-08"));
});

test("RT-08 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt08ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt08ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt08ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt08ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-08 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt08-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt08BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt08BoundFile("live/manifest.json", {
        root,
...
test("RT-08 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt08-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt08ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-08 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt08-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt08ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
