...
import {
  ...
  loadEvt02ResolvedManifest,
  readEvt02BoundFile,
  validateEvt02ResolvedManifest,
} from "../../scripts/evt02-governance.mjs";
import { publishEvt02ResolvedBytes } from "../../scripts/generate-evt02-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("EVT-02 resolved governance binds exact Cron Lux runtime and evidence identities", async () => {
  const built = await buildEvt02ResolvedManifest();
  const loaded = await loadEvt02ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateEvt02ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "EVT-02"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "EVT-02"));
});

test("EVT-02 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildEvt02ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateEvt02ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateEvt02ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateEvt02ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("EVT-02 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-evt02-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readEvt02BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readEvt02BoundFile("live/manifest.json", {
        root,
...
test("EVT-02 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-evt02-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishEvt02ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("EVT-02 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-evt02-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishEvt02ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
