...
import {
  ...
  loadRt06ResolvedManifest,
  readRt06BoundFile,
  validateRt06ResolvedManifest,
} from "../../scripts/rt06-governance.mjs";
import { publishRt06ResolvedBytes } from "../../scripts/generate-rt06-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-06 resolved governance binds exact runtime and cancellation evidence identities", async () => {
  const built = await buildRt06ResolvedManifest();
  const loaded = await loadRt06ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt06ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-06"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-06"));
});

test("RT-06 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt06ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt06ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt06ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt06ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-06 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt06-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt06BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt06BoundFile("live/manifest.json", {
        root,
...
test("RT-06 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt06-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt06ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-06 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt06-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt06ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
