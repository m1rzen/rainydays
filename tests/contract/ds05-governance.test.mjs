...
import {
  ...
  loadDs05ResolvedManifest,
  readDs05BoundFile,
  validateDs05ResolvedManifest,
} from "../../scripts/ds05-governance.mjs";
import { publishDs05ResolvedBytes } from "../../scripts/generate-ds05-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-05 resolved governance binds exact attachment runtime and evidence identities", async () => {
  const built = await buildDs05ResolvedManifest();
  const loaded = await loadDs05ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs05ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-05"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-05"));
});

test("DS-05 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs05ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs05ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs05ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs05ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-05 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds05-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs05BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs05BoundFile("live/manifest.json", {
        root,
...
test("DS-05 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds05-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs05ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-05 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds05-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs05ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
