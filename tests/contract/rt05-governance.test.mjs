...
import {
  ...
  loadRt05ResolvedManifest,
  readRt05BoundFile,
  validateRt05ResolvedManifest,
} from "../../scripts/rt05-governance.mjs";
import { publishRt05ResolvedBytes } from "../../scripts/generate-rt05-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-05 resolved governance binds exact runtime and cancellation evidence identities", async () => {
  const built = await buildRt05ResolvedManifest();
  const loaded = await loadRt05ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt05ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-05"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-05"));
});

test("RT-05 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt05ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt05ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt05ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt05ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-05 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt05-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt05BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt05BoundFile("live/manifest.json", {
        root,
...
test("RT-05 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt05-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt05ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-05 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt05-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt05ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
