...
import {
  ...
  loadRt11ResolvedManifest,
  readRt11BoundFile,
  validateRt11ResolvedManifest,
} from "../../scripts/rt11-governance.mjs";
import { publishRt11ResolvedBytes } from "../../scripts/generate-rt11-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("RT-11 resolved governance binds exact Session semantics runtime and evidence identities", async () => {
  const built = await buildRt11ResolvedManifest();
  const loaded = await loadRt11ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateRt11ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "RT-11"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "RT-11"));
});

test("RT-11 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildRt11ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateRt11ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateRt11ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateRt11ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("RT-11 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-rt11-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readRt11BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readRt11BoundFile("live/manifest.json", {
        root,
...
test("RT-11 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt11-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishRt11ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("RT-11 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-rt11-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishRt11ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
