...
import {
  ...
  loadEvt01ResolvedManifest,
  readEvt01BoundFile,
  validateEvt01ResolvedManifest,
} from "../../scripts/evt01-governance.mjs";
import { publishEvt01ResolvedBytes } from "../../scripts/generate-evt01-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("EVT-01 resolved governance binds exact EventBus runtime and evidence identities", async () => {
  const built = await buildEvt01ResolvedManifest();
  const loaded = await loadEvt01ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateEvt01ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "EVT-01"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "EVT-01"));
});

test("EVT-01 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildEvt01ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateEvt01ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateEvt01ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateEvt01ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("EVT-01 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-evt01-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readEvt01BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readEvt01BoundFile("live/manifest.json", {
        root,
...
test("EVT-01 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-evt01-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishEvt01ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("EVT-01 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-evt01-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishEvt01ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
