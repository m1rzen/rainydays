...
import {
  ...
  loadTool04ResolvedManifest,
  readTool04BoundFile,
  validateTool04ResolvedManifest,
} from "../../scripts/tool04-governance.mjs";
import { publishTool04ResolvedBytes } from "../../scripts/generate-tool04-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("TOOL-04 resolved governance binds exact isolated script runtime and evidence identities", async () => {
  const built = await buildTool04ResolvedManifest();
  const loaded = await loadTool04ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateTool04ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "TOOL-04"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "TOOL-04"));
});

test("TOOL-04 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildTool04ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateTool04ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateTool04ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateTool04ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("TOOL-04 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-tool04-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readTool04BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readTool04BoundFile("live/manifest.json", {
        root,
...
test("TOOL-04 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-tool04-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishTool04ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("TOOL-04 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-tool04-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishTool04ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
