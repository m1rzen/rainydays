...
import {
  ...
  loadDs04ResolvedManifest,
  readDs04BoundFile,
  validateDs04ResolvedManifest,
} from "../../scripts/ds04-governance.mjs";
import { publishDs04ResolvedBytes } from "../../scripts/generate-ds04-resolved-manifest.mjs";
import { makeTempDir, removeFixture } from "../helpers.mjs";

test("DS-04 resolved governance binds exact PTY/ConPTY runtime and evidence identities", async () => {
  const built = await buildDs04ResolvedManifest();
  const loaded = await loadDs04ResolvedManifest();
  assert.deepEqual(loaded.manifest, built);
  await validateDs04ResolvedManifest(loaded.manifest);
  assert(loaded.manifest.runtimeEntries.every(entry => entry.owner === "DS-04"));
  assert(loaded.manifest.testEntries.every(entry => entry.owner === "DS-04"));
});

test("DS-04 resolved governance rejects payload and file identity substitution", async () => {
  const built = await buildDs04ResolvedManifest();
  const payloadTamper = structuredClone(built);
  payloadTamper.testEntries[0].owner = "GOV-03";
  await assert.rejects(() => validateDs04ResolvedManifest(payloadTamper), /payload digest differs/u);

  const hashTamper = structuredClone(built);
  hashTamper.runtimeEntries[0].sha256 = "0".repeat(64);
  await assert.rejects(() => validateDs04ResolvedManifest(hashTamper), /payload digest differs/u);
  ...
  await assert.rejects(() => validateDs04ResolvedManifest(resigned), /differs from governed inputs/u);
});

test("DS-04 governed reads reject hardlinks and pathname replacement after open", async () => {
  const root = await makeTempDir("mini-lux-ds04-bound-file-");
  try {
    ...
    await assert.rejects(
      () => readDs04BoundFile("hardlinked.json", { root, field: "hardlink fixture" }),
    ...
    await assert.rejects(
      () => readDs04BoundFile("live/manifest.json", {
        root,
...
test("DS-04 resolved publication rejects temporary pathname replacement before atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds04-temp-replace-");
  const parent = path.join(root, "manifests");
  const output = path.join(parent, "resolved.json");
  await mkdir(parent);
  try {
    await assert.rejects(
      () => publishDs04ResolvedBytes(output, "{\"trusted\":true}\n", {
        root,
...
test("DS-04 resolved publication pins its parent identity across the atomic rename", async () => {
  const root = await makeTempDir("mini-lux-ds04-publish-");
  const parent = path.join(root, "manifests");
  ...
  try {
    await assert.rejects(
      () => publishDs04ResolvedBytes(output, "{\"candidate\":true}\n", {
        root,
        beforeRename: async () => {
