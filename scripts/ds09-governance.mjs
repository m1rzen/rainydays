...
export const sourceManifestPath = "tests/manifests/ds-09.json";
export const resolvedManifestPath = "tests/manifests/ds-09-resolved.json";
...
export async function readDs09BoundFile(relative, { root = projectRoot, field = "DS-09 governed file", afterSnapshot, afterOpen } = {}) {
...
async function loadSource(root) {
  const sourceFile = await boundFile(sourceManifestPath, root, "DS-09 source manifest");
  const source = JSON.parse(sourceFile.bytes.toString("utf8"));
  exactKeys(source, ["schemaVersion", "taskId", "baseline", "personaChain", "runtimeFiles", "coverageExemptions", "tests"], "DS-09 source manifest");
  assert.equal(source.schemaVersion, 1);
  assert.equal(source.taskId, "DS-09");
  assert.deepEqual(source.baseline, baseline);
  assert.deepEqual(source.personaChain, personaChain);
  assert(Array.isArray(source.runtimeFiles) && source.runtimeFiles.length > 0, "DS-09 runtimeFiles are empty");
  uniquePaths(source.runtimeFiles, "DS-09 runtime file");
  assert(source.coverageExemptions && typeof source.coverageExemptions === "object" && !Array.isArray(source.coverageExemptions), "DS-09 coverageExemptions must be an object");
  uniquePaths(Object.keys(source.coverageExemptions), "DS-09 coverage exemption");
  for (const [exactCasePath, exemption] of Object.entries(source.coverageExemptions)) {
    assert(source.runtimeFiles.includes(exactCasePath), `DS-09 coverage exemption is not an exact runtime file: ${exactCasePath}`);
    exactKeys(exemption, ["reason", "evidenceLayer", "evidence"], `DS-09 coverage exemption ${exactCasePath}`);
    assert.equal(typeof exemption.reason, "string");
    assert(exemption.reason.trim().length >= 12, `DS-09 coverage exemption reason is too short: ${exactCasePath}`);
    assert(layers.has(exemption.evidenceLayer), `DS-09 coverage exemption layer is invalid: ${exactCasePath}`);
    assert(Array.isArray(exemption.evidence) && exemption.evidence.length > 0, `DS-09 coverage exemption evidence is empty: ${exactCasePath}`);
    uniquePaths(exemption.evidence.map(entry => entry?.exactCasePath), `DS-09 coverage exemption evidence ${exactCasePath}`);
    for (const evidence of exemption.evidence) {
      exactKeys(evidence, ["exactCasePath", "layer"], `DS-09 coverage exemption evidence ${exactCasePath}`);
      assert.equal(evidence.layer, exemption.evidenceLayer, `DS-09 coverage exemption evidence layer differs: ${exactCasePath}`);
    }
  }
  assert(Array.isArray(source.tests) && source.tests.length > 0, "DS-09 tests are empty");
  uniquePaths(source.tests.map(entry => entry?.exactCasePath), "DS-09 test");
  for (const entry of source.tests) {
    exactKeys(entry, ["exactCasePath", "layer"], "DS-09 test entry");
    assert(layers.has(entry.layer), `DS-09 test layer is invalid: ${entry.layer}`);
  }
  ...
  for (const [runtimePath, exemption] of Object.entries(source.coverageExemptions)) {
    for (const evidence of exemption.evidence) {
      ...
      assert(testEntry, `DS-09 coverage exemption evidence is not a governed test: ${runtimePath}`);
      assert.equal(testEntry.layer, evidence.layer, `DS-09 coverage exemption evidence test layer differs: ${runtimePath}`);
    }
  }
  return { source, sourceFile };
}

export async function buildDs09ResolvedManifest({ root = projectRoot, sourceSnapshot = null } = {}) {
  const { source, sourceFile } = sourceSnapshot ?? await loadSource(root);
  const runtimeEntries = [];
  for (const exactCasePath of source.runtimeFiles) {
    const file = await boundFile(exactCasePath, root, "DS-09 runtime file");
    runtimeEntries.push({ exactCasePath, sha256: file.sha256, owner: "DS-09" });
  }
  const testEntries = [];
  for (const entry of source.tests) {
    const file = await boundFile(entry.exactCasePath, root, "DS-09 test");
    testEntries.push({ exactCasePath: entry.exactCasePath, sha256: file.sha256, kind: "test", layer: entry.layer, owner: "DS-09" });
  }
  const resolved = {
    schemaVersion: 1,
    taskId: "DS-09",
...
export async function validateDs09ResolvedManifest(manifest, { root = projectRoot, sourceSnapshot = null } = {}) {
  exactKeys(manifest, ["schemaVersion", "taskId", "sourceManifest", "runtimeEntries", "testEntries", "canonicalPayloadSha256"], "DS-09 resolved manifest");
  assert.equal(manifest.schemaVersion, 1);
  assert.equal(manifest.taskId, "DS-09");
  assert.match(manifest.canonicalPayloadSha256, /^[a-f0-9]{64}$/u);
  assert.equal(manifest.canonicalPayloadSha256, payloadDigest(manifest), "DS-09 resolved payload digest differs");
  const exactSource = sourceSnapshot ?? await loadSource(root);
  assert.deepEqual(manifest, await buildDs09ResolvedManifest({ root, sourceSnapshot: exactSource }), "DS-09 resolved manifest differs from governed inputs");
  return manifest;
}

export async function loadDs09ResolvedManifest({ root = projectRoot } = {}) {
  const sourceSnapshot = await loadSource(root);
  const file = await boundFile(resolvedManifestPath, root, "DS-09 resolved manifest");
  const manifest = JSON.parse(file.bytes.toString("utf8"));
  await validateDs09ResolvedManifest(manifest, { root, sourceSnapshot });
  return {
    manifest,
...
export function serializeDs09ResolvedManifest(manifest) {
