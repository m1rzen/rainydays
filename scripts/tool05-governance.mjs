...
export const sourceManifestPath = "tests/manifests/tool-05.json";
export const resolvedManifestPath = "tests/manifests/tool-05-resolved.json";
...
export async function readTool05BoundFile(relative, { root = projectRoot, field = "TOOL-05 governed file", afterSnapshot, afterOpen } = {}) {
...
async function loadSource(root) {
  const sourceFile = await boundFile(sourceManifestPath, root, "TOOL-05 source manifest");
  const source = JSON.parse(sourceFile.bytes.toString("utf8"));
  exactKeys(source, ["schemaVersion", "taskId", "baseline", "personaChain", "runtimeFiles", "coverageExemptions", "tests"], "TOOL-05 source manifest");
  assert.equal(source.schemaVersion, 1);
  assert.equal(source.taskId, "TOOL-05");
  assert.deepEqual(source.baseline, baseline);
  assert.deepEqual(source.personaChain, personaChain);
  assert(Array.isArray(source.runtimeFiles) && source.runtimeFiles.length > 0, "TOOL-05 runtimeFiles are empty");
  uniquePaths(source.runtimeFiles, "TOOL-05 runtime file");
  assert(source.coverageExemptions && typeof source.coverageExemptions === "object" && !Array.isArray(source.coverageExemptions), "TOOL-05 coverageExemptions must be an object");
  uniquePaths(Object.keys(source.coverageExemptions), "TOOL-05 coverage exemption");
  for (const [exactCasePath, exemption] of Object.entries(source.coverageExemptions)) {
    assert(source.runtimeFiles.includes(exactCasePath), `TOOL-05 coverage exemption is not an exact runtime file: ${exactCasePath}`);
    exactKeys(exemption, ["reason", "evidenceLayer", "evidence"], `TOOL-05 coverage exemption ${exactCasePath}`);
    assert.equal(typeof exemption.reason, "string");
    assert(exemption.reason.trim().length >= 12, `TOOL-05 coverage exemption reason is too short: ${exactCasePath}`);
    assert(layers.has(exemption.evidenceLayer), `TOOL-05 coverage exemption layer is invalid: ${exactCasePath}`);
    assert(Array.isArray(exemption.evidence) && exemption.evidence.length > 0, `TOOL-05 coverage exemption evidence is empty: ${exactCasePath}`);
    uniquePaths(exemption.evidence.map(entry => entry?.exactCasePath), `TOOL-05 coverage exemption evidence ${exactCasePath}`);
    for (const evidence of exemption.evidence) {
      exactKeys(evidence, ["exactCasePath", "layer"], `TOOL-05 coverage exemption evidence ${exactCasePath}`);
      assert.equal(evidence.layer, exemption.evidenceLayer, `TOOL-05 coverage exemption evidence layer differs: ${exactCasePath}`);
    }
  }
  assert(Array.isArray(source.tests) && source.tests.length > 0, "TOOL-05 tests are empty");
  uniquePaths(source.tests.map(entry => entry?.exactCasePath), "TOOL-05 test");
  for (const entry of source.tests) {
    exactKeys(entry, ["exactCasePath", "layer"], "TOOL-05 test entry");
    assert(layers.has(entry.layer), `TOOL-05 test layer is invalid: ${entry.layer}`);
  }
  ...
  for (const [runtimePath, exemption] of Object.entries(source.coverageExemptions)) {
    for (const evidence of exemption.evidence) {
      ...
      assert(testEntry, `TOOL-05 coverage exemption evidence is not a governed test: ${runtimePath}`);
      assert.equal(testEntry.layer, evidence.layer, `TOOL-05 coverage exemption evidence test layer differs: ${runtimePath}`);
    }
  }
  return { source, sourceFile };
}

export async function buildTool05ResolvedManifest({ root = projectRoot, sourceSnapshot = null } = {}) {
  const { source, sourceFile } = sourceSnapshot ?? await loadSource(root);
  const runtimeEntries = [];
  for (const exactCasePath of source.runtimeFiles) {
    const file = await boundFile(exactCasePath, root, "TOOL-05 runtime file");
    runtimeEntries.push({ exactCasePath, sha256: file.sha256, owner: "TOOL-05" });
  }
  const testEntries = [];
  for (const entry of source.tests) {
    const file = await boundFile(entry.exactCasePath, root, "TOOL-05 test");
    testEntries.push({ exactCasePath: entry.exactCasePath, sha256: file.sha256, kind: "test", layer: entry.layer, owner: "TOOL-05" });
  }
  const resolved = {
    schemaVersion: 1,
    taskId: "TOOL-05",
...
export async function validateTool05ResolvedManifest(manifest, { root = projectRoot, sourceSnapshot = null } = {}) {
  exactKeys(manifest, ["schemaVersion", "taskId", "sourceManifest", "runtimeEntries", "testEntries", "canonicalPayloadSha256"], "TOOL-05 resolved manifest");
  assert.equal(manifest.schemaVersion, 1);
  assert.equal(manifest.taskId, "TOOL-05");
  assert.match(manifest.canonicalPayloadSha256, /^[a-f0-9]{64}$/u);
  assert.equal(manifest.canonicalPayloadSha256, payloadDigest(manifest), "TOOL-05 resolved payload digest differs");
  const exactSource = sourceSnapshot ?? await loadSource(root);
  assert.deepEqual(manifest, await buildTool05ResolvedManifest({ root, sourceSnapshot: exactSource }), "TOOL-05 resolved manifest differs from governed inputs");
  return manifest;
}

export async function loadTool05ResolvedManifest({ root = projectRoot } = {}) {
  const sourceSnapshot = await loadSource(root);
  const file = await boundFile(resolvedManifestPath, root, "TOOL-05 resolved manifest");
  const manifest = JSON.parse(file.bytes.toString("utf8"));
  await validateTool05ResolvedManifest(manifest, { root, sourceSnapshot });
  return {
    manifest,
...
export function serializeTool05ResolvedManifest(manifest) {
