...
async function main() {
  ...
  try {
    ...
    await writeFile(path.join(unsupportedMetadataRoot, "build-info.json"), JSON.stringify({
      ...secondBuild,
      versions: { ...secondBuild.versions, databaseSchema: 6 },
    }));
    ...
    pass("semantically unsupported metadata rejected", "database schema 6 rejected before runtime initialization");
    ...
    assert.equal(emptyDatabaseRun.payload?.userVersion, 5);
    ...
    assert.equal(crashZeroRecovery.payload?.userVersion, 5);
    ...
    assert.equal(freshRun.payload?.userVersion, 5);
    pass("fresh database migrates 0 to 5", "user_version=5");
    ...
    assert.equal(lifecycleRestart.payload?.userVersion, 5);
    ...
    pass("legitimate memo and session writes survive database restart", "Schema 5 reopens with session and memo rows preserved");
    pass("Schema 5 repeated startup is a migration no-op", "user_version, complete sqlite_master and sentinel rows remain logically identical");
    ...
    database.close();
    pass("legacy database migrates 0 to 5 without data loss", "sentinel row, audit table, Task DAG, EventBus, and Cron target schema preserved");
    ...
    pass("Schema 1 migrates transactionally to 5", "sentinel row preserved and exact audit, Task DAG, EventBus, and Cron target objects installed");
    ...
    database = new Database(futurePath); database.pragma("user_version = 6"); database.close();
    ...
    assert.equal(database.pragma("user_version", { simple: true }), 6); database.close();
    ...
    try {
      ...
      assert.equal(diagnostics.databaseSchemaVersion, 5);
