...
async function main() {
  ...
  try {
    ...
    pass("legitimate memo and session writes survive database restart", "Schema 3 reopens with session and memo rows preserved");
    pass("Schema 3 repeated startup is a migration no-op", "user_version, complete sqlite_master and sentinel rows remain logically identical");
    ...
    pass("Schema 1 migrates transactionally to 3", "sentinel row preserved and exact audit and Task DAG objects installed");
    ...
    database = new Database(futurePath); database.pragma("user_version = 4"); database.close();
    ...
    assert.equal(database.pragma("user_version", { simple: true }), 4); database.close();
