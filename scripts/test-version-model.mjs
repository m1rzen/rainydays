...
async function main() {
  ...
  try {
    ...
    await writeFile(path.join(unsupportedMetadataRoot, "build-info.json"), JSON.stringify({
      ...secondBuild,
      versions: { ...secondBuild.versions, databaseSchema: 10 },
    }));
    ...
    pass("semantically unsupported metadata rejected", "database schema 10 rejected before runtime initialization");
    ...
    assert.equal(emptyDatabaseRun.payload?.userVersion, 9);
    ...
    assert.equal(crashZeroRecovery.payload?.userVersion, 9);
    ...
    assert.equal(freshRun.payload?.userVersion, 9);
    pass("fresh database migrates 0 to 9", "user_version=9");
    ...
    assert.equal(lifecycleRestart.payload?.userVersion, 9);
    ...
    pass("legitimate memo and session writes survive database restart", "Schema 9 reopens with session, memo and workbench layout rows preserved");
    pass("Schema 9 repeated startup is a migration no-op", "user_version, complete sqlite_master and sentinel rows remain logically identical");
    ...
    database.close();
    pass("legacy database migrates 0 to 9 without data loss", "sentinel row, audit table, Task DAG, EventBus, Cron target, Poll subscription, and Session memo schema preserved");
    ...
    pass("Schema 1 migrates transactionally to 9", "sentinel row preserved and exact audit, Task DAG, EventBus, Cron target, Poll, and Session memo objects installed");
    ...
    database = new Database(futurePath); database.pragma("user_version = 9"); database.close();
    ...
    assert.equal(database.pragma("user_version", { simple: true }), 9); database.close();
    ...
    try {
      ...
      assert.equal(diagnostics.databaseSchemaVersion, 9);
