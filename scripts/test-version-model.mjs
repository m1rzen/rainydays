...
async function main() {
  ...
  try {
    ...
    await writeFile(path.join(unsupportedMetadataRoot, "build-info.json"), JSON.stringify({
      ...secondBuild,
      versions: { ...secondBuild.versions, databaseSchema: 9 },
    }));
    ...
    pass("semantically unsupported metadata rejected", "database schema 9 rejected before runtime initialization");
    ...
    assert.equal(emptyDatabaseRun.payload?.userVersion, 8);
    ...
    assert.equal(crashZeroRecovery.payload?.userVersion, 8);
    ...
    assert.equal(freshRun.payload?.userVersion, 8);
    pass("fresh database migrates 0 to 8", "user_version=8");
    ...
    assert.equal(lifecycleRestart.payload?.userVersion, 8);
    ...
    pass("legitimate memo and session writes survive database restart", "Schema 8 reopens with session, memo and workbench layout rows preserved");
    pass("Schema 8 repeated startup is a migration no-op", "user_version, complete sqlite_master and sentinel rows remain logically identical");
    ...
    database.close();
    pass("legacy database migrates 0 to 8 without data loss", "sentinel row, audit table, Task DAG, EventBus, Cron target, Poll subscription, and Session memo schema preserved");
    ...
    pass("Schema 1 migrates transactionally to 8", "sentinel row preserved and exact audit, Task DAG, EventBus, Cron target, Poll, and Session memo objects installed");
    ...
    try {
      ...
      assert.equal(diagnostics.databaseSchemaVersion, 8);
